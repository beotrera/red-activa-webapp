...
import { useAudioRecorder, AUDIO_MAX_DURATION_SECONDS } from "../hooks/useAudioRecorder";
import { toast } from "sonner";
import { Hospital, Camera, Image, X, ArrowLeft, Mic, Square, Trash2, AlertCircle } from "lucide-react";
    ...
    createNNMutation.mutate(
      {
        ...
        audio: recorder.audioFile ?? undefined,
      },
      {
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-sm">
          ...
          <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-3">
            <div className="flex items-center gap-1.5 border-b border-slate-200 pb-2">
              ...
              <span className="text-xs font-semibold text-slate-700">Nota de voz (opcional)</span>
              <span className="ml-auto text-[10px] text-slate-400">Máx. 2 min · 5 MB</span>
            </div>
            ...
            {recorder.state === "idle" && (
              <button
                ...
                className="w-full flex items-center justify-center gap-2 px-3 py-2.5 bg-white hover:bg-slate-100 border border-slate-200 text-slate-700 text-sm font-medium rounded-lg transition cursor-pointer"
              >
            ...
            {recorder.state === "requesting" && (
              <div className="text-center text-xs text-slate-400 py-2">Solicitando acceso al micrófono...</div>
            )}
            ...
            {recorder.state === "recording" && (
              <div className="flex items-center justify-between bg-red-50 border border-red-200 rounded-lg px-3 py-2.5">
                <span className="flex items-center gap-2 text-sm font-semibold text-red-700">
                  ...
                  Grabando... {formatElapsed(recorder.elapsedSeconds)} / {formatElapsed(AUDIO_MAX_DURATION_SECONDS)}
                </span>
                <button
                  type="button"
                  onClick={recorder.stop}
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-red-600 hover:bg-red-700 text-white text-xs font-bold rounded-lg transition cursor-pointer"
                >
            ...
            {recorder.state === "recorded" && recorder.audioUrl && (
              <div className="flex items-center gap-2">
                <audio controls src={recorder.audioUrl} className="flex-1 h-10" />
                <button
                  type="button"
                  onClick={recorder.discard}
                  title="Descartar grabación"
                  className="p-2 bg-white hover:bg-red-50 border border-slate-200 text-slate-500 hover:text-red-600 rounded-lg transition cursor-pointer shrink-0"
                >
